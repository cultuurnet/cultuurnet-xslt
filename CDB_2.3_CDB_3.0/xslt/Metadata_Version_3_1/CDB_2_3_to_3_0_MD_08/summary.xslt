...
-->
				...
				<xsl:when test="$group_1 != '' or $group_2 != '' or $group_3 != '' or $group_4 != '' or $group_5 != '' or $group_6 != ''">
					...
					<xsl:choose>
						<xsl:when test="substring($group_1_times,1,29) = 'mo, di, woe, do, vrij, za, zo'">
							<xsl:value-of select="'ma tot zo '"/>
							<xsl:value-of select="$times_mon"/>
						</xsl:when>
						<xsl:when test="substring($group_1_times,1,25) = 'mo, di, woe, do, vrij, za'">
							<xsl:value-of select="'ma tot za '"/>
							<xsl:value-of select="$times_mon"/>
						</xsl:when>
						<xsl:when test="substring($group_1_times,1,21) = 'mo, di, woe, do, vrij'">
					...
					<xsl:if test="string($group_2) = '' 
												and string($times_tue) !='' and not(contains($all_group_times,$times_tue))">
						<xsl:value-of select="'&#xA;'"/>
						<xsl:value-of select="'di '"/>
						<xsl:value-of select="$times_tue"/>
					</xsl:if>
					<xsl:if test="string($group_3) = '' and
												  string($times_wed) != '' and not(contains($all_group_times,$times_wed))">
						<xsl:value-of select="'&#xA;'"/>
						<xsl:value-of select="'woe '"/>
						<xsl:value-of select="$times_wed"/>
					</xsl:if>
					<xsl:if test="string($group_4) = '' 
												 and string($times_thu) !='' and not(contains($all_group_times,$times_thu))">
						<xsl:value-of select="'&#xA;'"/>
						<xsl:value-of select="'do '"/>
						<xsl:value-of select="$times_thu"/>
					</xsl:if>
					<xsl:if test="string($group_5) = '' 
												and string($times_fri) !='' and not(contains($all_group_times,$times_fri))">
						<xsl:value-of select="'&#xA;'"/>
						<xsl:value-of select="'vrij '"/>
						<xsl:value-of select="$times_fri"/>
					</xsl:if>
					...
					bbbbbbb $group_3  <xsl:value-of select="$group_6"/>
