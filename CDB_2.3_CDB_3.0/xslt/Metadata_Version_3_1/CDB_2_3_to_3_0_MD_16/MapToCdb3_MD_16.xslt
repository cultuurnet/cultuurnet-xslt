												...
												<xsl:when test="substring(./file,($media_length)-3,4) = '.txt'">
													...
													<xsl:if test="string(./value_plain_text/dutch) != 'kinderen toegelaten'">
														...
														<xsl:if test="string(./category/categorisation_instance/label/dutch) != 'prijsomschrijvingen en kortingen'">
															...
															<xsl:variable name="url_1" select="concat('http://www.cultuurweb.be/CNETPortal/media/',$text_file)"/>
															...
															<xsl:variable name="url_3" select="concat('http://media.cultuurnet.be/media/',$text_file)"/>
															<!-- check the url exists -->
															...
															<!--
															xxxxx
															<xsl:value-of select="$test_URL_1"></xsl:value-of>aaa
															<xsl:value-of select="$test_URL_2"></xsl:value-of>bbb
															<xsl:value-of select="$test_URL_3"></xsl:value-of>ccc
															zzzzz
															<xsl:value-of select="$url_1"></xsl:value-of>
															<xsl:value-of select="$url_2"></xsl:value-of>
															<xsl:value-of select="$url_3"></xsl:value-of>
															yyyyyy
-->
															<xsl:variable name="file" select="java:java.io.File.new($url_1)"/>
															<xsl:if test="position() = 1">
																<xsl:choose>
																	<xsl:when test="$test_URL_1 = 'true'">
																		<xsl:variable name="text_string">
																			<xsl:value-of select="extfun:writeLoop(string($url_1))"/>
																		</xsl:variable>
																	...
																	<xsl:when test="$test_URL_2 = 'true'">
																		<xsl:variable name="text_string">
																			<xsl:value-of select="extfun:writeLoop(string($url_2))"/>
																		</xsl:variable>
																		<xsl:variable name="adjust_1">
																			<xsl:value-of select="translate($text_string,'þ','&#xA;')"/>
																		</xsl:variable>
																	...
																	<xsl:when test="$test_URL_3 = 'true'">
																		<xsl:variable name="text_string">
																			<xsl:value-of select="extfun:writeLoop(string($url_3))"/>
																		</xsl:variable>
																		<xsl:variable name="adjust_1">
																			<xsl:value-of select="translate($text_string,'þ','&#xA;')"/>
																		</xsl:variable>
