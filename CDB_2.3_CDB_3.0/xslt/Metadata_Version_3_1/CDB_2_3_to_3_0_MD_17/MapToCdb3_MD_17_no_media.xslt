											...
											<xsl:when test="substring(./file,($media_length)-3,4) = '.txt'">
												...
												<xsl:if test="string(./value_plain_text/dutch) != 'kinderen toegelaten'">
													...
													<xsl:if test="string(./category/categorisation_instance/label/dutch) != 'prijsomschrijvingen en kortingen'">
														...
														<xsl:variable name="local_dir">
															<xsl:value-of select="java:org.apache.xalan.lib.NodeInfo.systemId()"/>
														</xsl:variable>
														xxx<xsl:value-of select="$local_dir"></xsl:value-of>zzz

														<!--  xxx<xsl:value-of select="xalan:systemId()"></xsl:value-of>zzz  -->
																<xsl:variable name="full_text_file" select="concat('D:\XML\XML\SKB\SKB00001_200905052300_0001_D\',$text_file)"></xsl:variable>
