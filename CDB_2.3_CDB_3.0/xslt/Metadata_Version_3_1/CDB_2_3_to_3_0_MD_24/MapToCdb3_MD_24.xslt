...
<!--
Not for use in IIS1 or IIS2
Maps CDB 2.3 to CDB 3.0
This version takes .txt files from the same folder as the xml file stores them and copies them into the longdescription part of the CDB XML 3.1
Nothing is done with media files. Use Copy_Media_pub_02.xslt for that.
Use this for Socius and SKB imports.
-->
<xsl:stylesheet version="1.0" xmlns="http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.1/DRAFT" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:cc="http://www.cultuurdatabank.be/XMLSchema/Culture_CS" xmlns:sc="http://www.cultuurdatabank.be/XMLSchema/SA_comp" xmlns:ad="http://www.cultuurdatabank.be/XMLSchema/address" xmlns:ca="http://www.cultuurdatabank.be/XMLSchema/categorisation" xmlns:ci="http://www.cultuurdatabank.be/XMLSchema/city_area" xmlns:ct="http://www.cultuurdatabank.be/XMLSchema/city_comp" xmlns:co="http://www.cultuurdatabank.be/XMLSchema/country" xmlns:ec="http://www.cultuurdatabank.be/XMLSchema/event_cluster" xmlns:ed="http://www.cultuurdatabank.be/XMLSchema/event_date" xmlns:gc="http://www.cultuurdatabank.be/XMLSchema/gis_comp" xmlns:lc="http://www.cultuurdatabank.be/XMLSchema/language_comp" xmlns:mc="http://www.cultuurdatabank.be/XMLSchema/mail_comp" xmlns:me="http://www.cultuurdatabank.be/XMLSchema/media" xmlns:md="http://www.cultuurdatabank.be/XMLSchema/metadata" xmlns:na="http://www.cultuurdatabank.be/XMLSchema/name_comp" xmlns:no="http://www.cultuurdatabank.be/XMLSchema/note_comp" xmlns:ot="http://www.cultuurdatabank.be/XMLSchema/opening_times" xmlns:or="http://www.cultuurdatabank.be/XMLSchema/organisation" xmlns:pe="http://www.cultuurdatabank.be/XMLSchema/person" xmlns:ph="http://www.cultuurdatabank.be/XMLSchema/phone" xmlns:pr="http://www.cultuurdatabank.be/XMLSchema/price" xmlns:ref="http://www.cultuurdatabank.be/XMLSchema/reference_item" xmlns:rn="http://www.cultuurdatabank.be/XMLSchema/region_nat" xmlns:rel="http://www.cultuurdatabank.be/XMLSchema/relation_item" xmlns:sp="http://www.cultuurdatabank.be/XMLSchema/street_part" xmlns:uc="http://www.cultuurdatabank.be/XMLSchema/url_comp" xmlns:zc="http://www.cultuurdatabank.be/XMLSchema/zip_codes" xmlns:java="http://xml.apache.org/xalan/java" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:extfun="external_functions" xmlns:file="java.io.File" xmlns:xalan="http://xml.apache.org/xalan" exclude-result-prefixes="cc sc ad ca ci ct co ec ed gc lc mc me md na no ot or pe ph pr ref rn rel sp uc zc xs xalan java extfun file">
   ...
   //  java.lang.System.out.println('  -----');
    if(RegExp.test(url)){ 
		...
		//all_text_1 = all_text.replace(/;B;/g,'')
		//all_text_1= all_text.replace(/;39;/g,"'")
		//return all_text_1
...
\\172.16.1.51\xml\XSD\CDB_3_0\CDBXML~1.0-F\CdbXSD.xsd</xsl:attribute>-->
		...
		http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.1/DRAFT/CdbXSD.xsd</xsl:attribute>
			...
			<xsl:if test="count(//event_cluster_instance) != 0">
				<!--  <xsl:variable name="all_events">  -->
				<xsl:variable name="select_events">
					<!--<events> -->
							...
							<xsl:for-each select="organisation">
								<xsl:for-each select="or:organisation">
									<xsl:if test="rel:relation_item/relation_item_instance/categorisation_label = 'lokatie'">
										<xsl:if test="string(organisation_instance/address) != '' and string(organisation_instance/address/ad:address/address_instance/city_free_text) != '' and string(organisation_instance/address/ad:address/address_instance/zip_codes) != '0000' and string(organisation_instance/address/ad:address/address_instance/zip_codes) != ''">
											<xsl:value-of select="'true'"/>
										</xsl:if>
									</xsl:if>
								</xsl:for-each>
							...
							<event>
								...
								<xsl:choose>
									<xsl:when test="string(event_base_type) = 'tijdelijk'">
										<xsl:choose>
											<xsl:when test="string(md:availableto) != ''">
												<xsl:attribute name="availableto"><xsl:value-of select="md:availableto"/></xsl:attribute>
											</xsl:when>
											<xsl:when test="string(md:availableto) = '' and string(date_to) != ''">
												<xsl:attribute name="availableto"><xsl:value-of select="date_to"/></xsl:attribute>
											</xsl:when>
											<xsl:otherwise>
												<!-- not needed -->
											</xsl:otherwise>
								...
								<!-- add private -->
								<xsl:variable name="ocmw">
									<xsl:for-each select="category">
										<xsl:for-each select="categorisation_instance">
											<xsl:if test="string(md:objectid) = '0.50.15.0.0' or
											string(md:objectid) = '0.50.15.1.0' or
											string(md:objectid) = '0.50.15.2.0' or
											string(md:objectid) = '0.50.15.3.0' or
											string(md:objectid) = '0.50.15.4.0' or
											string(md:objectid) = '0.50.15.5.0' or
											string(md:objectid) = '0.50.15.6.0' or
											string(md:objectid) = '0.50.15.7.0' or
											string(md:objectid) = '0.50.15.8.0' or
											string(md:objectid) = '0.50.15.9.0' or
											string(md:objectid) = '0.50.15.10.0' or
											string(md:objectid) = '0.50.15.11.0' or
											string(md:objectid) = '0.50.15.12.0' or
											string(md:objectid) = '0.50.15.13.0' or
											string(md:objectid) = '0.50.15.14.0' or
											string(md:objectid) = '0.38.0.0.0' or
											string(md:objectid) = '0.40.0.0.0'">
												<xsl:value-of select="'true'"/>
											</xsl:if>
										</xsl:for-each>
									</xsl:for-each>
								...
								<xsl:variable name="private">
									...
									<xsl:if test="substring($ocmw,1,4) = 'true'">
										<xsl:value-of select="'true'"/>
									</xsl:if>
									<xsl:if test="string(md:public_type) = '0'">
										<xsl:value-of select="'true'"/>
									</xsl:if>
								</xsl:variable>
								<xsl:if test="substring($private,1,4) = 'true'">
									<xsl:attribute name="private"><xsl:value-of select="true()"/></xsl:attribute>
								</xsl:if>
								...
								<xsl:choose>
									...
									<xsl:otherwise>
										...
										<xsl:choose>
											<xsl:when test="string($org_contact) != ''">
												<xsl:copy-of select="$org_contact"/>
											</xsl:when>
											<xsl:when test="string($per_contact) != ''">
												<xsl:copy-of select="$per_contact"/>
											</xsl:when>
											<xsl:when test="string($loc_contact) != ''">
												<xsl:copy-of select="$loc_contact"/>
											</xsl:when>
											<xsl:otherwise>
												<contactinfo>
													<phone>+3225511870
								...
								<eventdetails>
									<eventdetail>
										...
										<xsl:variable name="executor_exists">
											...
											<!-- updated so we don't use the organisator as a performer -->
											<xsl:for-each select="person">
												<xsl:for-each select="pe:person">
													<xsl:if test="string(rel:relation_item/relation_item_instance/categorisation_label) != 'organisator'">
														<xsl:value-of select="1"/>
													</xsl:if>
												</xsl:for-each>
											</xsl:for-each>
...
$uppercase), substring(rel:relation_item/relation_item_instance/relationdesc,2))"/>
																...
																<xsl:for-each select="organisation_instance">
																	...
																	<xsl:variable name="type_onbepaald">
																		<xsl:for-each select="category/categorisation_instance">
																			<xsl:if test="string(label/dutch) = 'Type organisatie onbepaald'">
																				<xsl:value-of select="'true'"/>
																			</xsl:if>
																		</xsl:for-each>
																	</xsl:variable>
																	...
																	<xsl:choose>
																		<xsl:when test="string($actor_type) = 'reference' or
																									string($thema_onbepaald) = 'true' or 
																									string(md:short_description/dutch) = 'NB' or
																									string(md:short_description/dutch) = 'Beschrijving niet beschikbaar' or
																									string(md:short_description/dutch) = 'Beschrijving niet beschikbaar.' or
																									string(md:short_description/dutch) = 'Niet beschikbaar'  or
																									string(md:short_description/dutch) = '' or
																									string(category) = '' or
																									string($type_onbepaald) = 'true' or
																									string(md:dummy) = '1'">
																			<!-- used for dummy actors -->
...
$uppercase), substring(rel:relation_item/relation_item_instance/relationdesc,2))"/>
										...
										<!-- long description -->
										<!-- what to do if a long description in .txt file exists : 							-->
										<!-- if .txt file is in media_instance and it exists in the file then use it 	-->
										<!-- if not then check in the CDB media files 											-->
												...
												<xsl:when test="substring(./file,($media_length)-3,4) = '.txt'">
													...
													<xsl:if test="string(./value_plain_text/dutch) != 'kinderen toegelaten'">
														...
														<xsl:if test="string(./category/categorisation_instance/label/dutch) != 'prijsomschrijvingen en kortingen'">
															...
															<!-- so use chop off the file: and use everything up to the last / -->
															<xsl:variable name="local_dir_1">
																<!-- <xsl:value-of select="'file:/opt/iis3/publications/46/SKB_Import_Delta_090507171801449_processing/transform1_SKB_adjust_lab_url_pho_rel_price.temp'"/> -->
																<!-- this finds the name of the directory where the xml is -->
															...
															<xsl:variable name="local_dir_3">
																c and now select everything up to and including the last / 
																c IIS3 	<sl:value-of select="substring($local_dir_2,1,70)"/> 
																c BE PC <xsl:value-of select="substring($local_dir_2,1,44)"/> 
																<xsl:value-of select="substring($local_dir_2,1,44)"/>
															</xsl:variable>-->
															<xsl:variable name="local_dir_3">
															...
															<xsl:variable name="full_text_file" select="concat($local_dir_3,$text_file)"/>
															...
															<xsl:if test="position() = 1">
																<xsl:choose>
																	...
																	<!-- Old. Changed  on 8/6/11 to translate some bad characters in the long descriptions. 
																		<longdescription>
																			<xsl:value-of select="extfun:get_text_file($full_text_file)"/>
																		</longdescription>
																	could do this in one long line but this is clearer -->
																	
																	<xsl:variable name="longdesc_01">
																		<xsl:value-of select="extfun:get_text_file($full_text_file)"/>
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_02">
																		<xsl:value-of select="translate($longdesc_01,'&#150;','-')"/>
																	</xsl:variable>
																	
																	<xsl:variable name="apos" select='"&apos;"' />
																	<xsl:variable name="longdesc_03">
																		<xsl:value-of select="translate($longdesc_02,'&#146;',$apos)"/>
																		<!-- can't use this																						-->
																		<!--  <xsl:value-of select="translate($longdesc_02,'&#146;','&apos;')"/> 	-->
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_04">
																		<xsl:value-of select="translate($longdesc_03,'&#145;',$apos)"/>
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_05">
																		<xsl:value-of select="translate($longdesc_04,'&#147;','&quot;')"/>
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_06">
																		<xsl:value-of select="translate($longdesc_05,'&#148;','&quot;')"/>
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_07">
																		<xsl:value-of select="translate($longdesc_06,'&#149;','-')"/>
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_08">
																		<xsl:value-of select="translate($longdesc_07,'&#151;','-')"/>
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_09">
																		<xsl:value-of select="translate($longdesc_08,'&#132;','&quot;')"/>
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_10">
																		<xsl:call-template name="replace_characters">
																			<xsl:with-param name="string" select="$longdesc_09"/>
																			<xsl:with-param name="search" select="'&#133;'"/>
																			<xsl:with-param name="replace" select="'...'"/>
																		</xsl:call-template>
																	</xsl:variable>
																	
																	<xsl:variable name="longdesc_11">
																		<xsl:call-template name="replace_characters">
																			<xsl:with-param name="string" select="$longdesc_10"/>
																			<xsl:with-param name="search" select="'&#128;'"/>
																			<xsl:with-param name="replace" select="'euro'"/>
																		</xsl:call-template>
																	</xsl:variable>
																	<longdescription>
																			<xsl:value-of select="$longdesc_11"/>
																	</longdescription>
																	</xsl:when>
																	<!-- removed to speed up processing of historical data
																	<xsl:otherwise>xxxxx
																		comment in this case copy the long desctiption from one of the CDB libraries	
																		comment check this with a 2.3 file from the CDB 												
																		<xsl:variable name="test_file_url">
																			<xsl:variable name="url_1" select="concat('http://media.cultuurnet.be/media/',$text_file)"/>
																			<xsl:variable name="test_URL_1">
																				<xsl:value-of select="extfun:testURL(string($url_1))"/>
																			</xsl:variable>
																			<xsl:choose>
																				<xsl:when test="string($test_URL_1) = 'true'">
																					<xsl:value-of select="$url_1"/>
																				</xsl:when>
																				<xsl:otherwise>
																					<xsl:variable name="url_2" select="concat('http://build.uitdatabank.be/media/',$text_file)"/>
																					<xsl:variable name="test_URL_2">
																						<xsl:value-of select="extfun:testURL(string($url_2))"/>
																					</xsl:variable>
																					<xsl:choose>
																						<xsl:when test="string($test_URL_2) = 'true'">
																							<xsl:value-of select="$url_2"/>
																						</xsl:when>
																						<xsl:otherwise>
																							<xsl:variable name="url_3" select="concat('http://212.117.81.133/igo_dyn/igomedia/',$text_file)"/>
																							<xsl:variable name="test_URL_3">
																								<xsl:value-of select="extfun:testURL(string($url_3))"/>
																							</xsl:variable>
																							<xsl:choose>
																								<xsl:when test="string($test_URL_3) = 'true'">
																									<xsl:value-of select="$url_3"/>
																								</xsl:when>
																								<xsl:otherwise>
																									<xsl:variable name="url_4" select="concat('http://www.cultuurweb.be/CNETPortal/media/',$text_file)"/>
																									<xsl:variable name="test_URL_4">
																										<xsl:value-of select="extfun:testURL(string($url_4))"/>
																									</xsl:variable>
																									<xsl:choose>
																										<xsl:when test="string($test_URL_4) = 'true'">
																		...
																		<xsl:if test="position() = 1">
																			<xsl:choose>
																				<xsl:when test="string($test_file_url) != ''">
																					<xsl:variable name="text_string">
																						<xsl:value-of select="extfun:writeLoop(string($test_file_url))"/>
																					</xsl:variable>
										...
										<!-- media -->
										<!-- don't include text files. They have already been copied above-->
										...
										<!-- 12/02/10 update the hlink to http: //media.cultuurnet.be/partnermedia/vlieg.jpg -->
											...
											<media>
												<xsl:if test="substring($vlieg,1,4) = 'true'">
													<file>
														...
														<hlink>http://media.cultuurnet.be/partnermedia/vlieg.jpg</hlink>
...
-->
																...
																<hlink>
																	<!-- old 11/01/2011 <xsl:value-of select="'http://media.cultuurnet.be/partnermedia/'"/> -->
														...
														<price>
															...
															string(me:media/media_instance/value_plain_text/dutch) != ''">
																	<pricedescription>
																		...
																		<xsl:value-of select="translate(me:media/media_instance/value_plain_text/dutch,'&#145;,&#146;,&#128;,&#133;,&#150;','&#8216;,&#8217;,&#8364;,&#8230;,-')"/>
																	</pricedescription>
																</xsl:when>
																<xsl:otherwise>
																	<xsl:for-each select="../../me:media/media_instance">
																		<xsl:if test="string(category/categorisation_instance/label/dutch) = 'prijsomschrijvingen en kortingen'">
																			<pricedescription>
																				<!-- <xsl:value-of select="value_plain_text/dutch"/> -->
																				<xsl:value-of select="translate(value_plain_text/dutch,'&#145;,&#146;,&#128;,&#133;,&#150;','&#8216;,&#8217;,&#8364;,&#8230;,-')"/>
																			</pricedescription>
																		</xsl:if>
																	</xsl:for-each>
																</xsl:otherwise>
								...
								<!-- event relations. this is where references to a production or koepelevent go in exports from CDB-->
								<!-- addded for MD_03 -->
								<xsl:choose>
									<xsl:when test="string(productieid) != '' or string(koepelid) != ''">
										<eventrelations>
									...
									<xsl:otherwise>
										<!-- event relations. this is where references to a production or koepelevent go in data from partnere, eg TA, SKB -->
														...
														<xsl:for-each select="organisation_instance">
															...
															<xsl:choose>
																<xsl:when test="string($actor_type) = 'reference' or
																	...
																	<!-- <xsl:call-template name="insertActor_ref"/> -->
																	<!-- use this otherwise we loose SKB, TA and SOCIUS data -->
																	<xsl:call-template name="insertActor"/>
																	<!-- </locationpart> -->
																</xsl:when>
																<xsl:otherwise>
				...
				<xsl:if test="string($select_events) != ''">
					<events>
						<xsl:copy-of select="$select_events"/>
					</events>
				</xsl:if>
				<!--
				</xsl:variable>
				<xsl:if test="string($all_events) != ''">
					<events>
						<xsl:copy-of select="$all_events"/>
					</events>
				</xsl:if> -->
			</xsl:if>
			<!-- add the producuions -->
			...
			<!-- deletes -->
			<!-- add the actors -->
			<!-- this is just goung to be used for 2.3 to 3?.1 actors so ther will be no references -->
			<xsl:if test="string(or:organisation) != ''">
				<actors>
					<xsl:for-each select="or:organisation">
						<!-- <xsl:if test="rel:relation_item/relation_item_instance/categorisation_label = 'lokatie' and position() = 1">-->
						<!-- <xsl:if test="position() = 1"> -->
	...
	<!-- to replace a bad character with more han one character  -->
	<xsl:template name="replace_characters">
		<xsl:param name="string"/>
		<xsl:param name="search"/>
		<xsl:param name="replace"/>
		<xsl:choose>
			<xsl:when test="contains(string($string),$search)">
				<xsl:value-of select="substring-before(string($string),$search)"/>
				<xsl:value-of select="$replace"/>
				<xsl:call-template name="replace_characters">
					<xsl:with-param name="string" select="substring-after(string($string),$search)"/>
