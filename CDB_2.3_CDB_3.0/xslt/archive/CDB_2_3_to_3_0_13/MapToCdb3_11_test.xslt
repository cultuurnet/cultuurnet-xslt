...
<xsl:stylesheet version="1.0" xmlns="http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.0/FINAL" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:cc="http://www.cultuurdatabank.be/XMLSchema/Culture_CS" xmlns:sc="http://www.cultuurdatabank.be/XMLSchema/SA_comp" xmlns:ad="http://www.cultuurdatabank.be/XMLSchema/address" xmlns:ca="http://www.cultuurdatabank.be/XMLSchema/categorisation" xmlns:ci="http://www.cultuurdatabank.be/XMLSchema/city_area" xmlns:ct="http://www.cultuurdatabank.be/XMLSchema/city_comp" xmlns:co="http://www.cultuurdatabank.be/XMLSchema/country" xmlns:ec="http://www.cultuurdatabank.be/XMLSchema/event_cluster" xmlns:ed="http://www.cultuurdatabank.be/XMLSchema/event_date" xmlns:gc="http://www.cultuurdatabank.be/XMLSchema/gis_comp" xmlns:lc="http://www.cultuurdatabank.be/XMLSchema/language_comp" xmlns:mc="http://www.cultuurdatabank.be/XMLSchema/mail_comp" xmlns:me="http://www.cultuurdatabank.be/XMLSchema/media" xmlns:md="http://www.cultuurdatabank.be/XMLSchema/metadata" xmlns:na="http://www.cultuurdatabank.be/XMLSchema/name_comp" xmlns:no="http://www.cultuurdatabank.be/XMLSchema/note_comp" xmlns:ot="http://www.cultuurdatabank.be/XMLSchema/opening_times" xmlns:or="http://www.cultuurdatabank.be/XMLSchema/organisation" xmlns:pe="http://www.cultuurdatabank.be/XMLSchema/person" xmlns:ph="http://www.cultuurdatabank.be/XMLSchema/phone" xmlns:pr="http://www.cultuurdatabank.be/XMLSchema/price" xmlns:ref="http://www.cultuurdatabank.be/XMLSchema/reference_item" xmlns:rn="http://www.cultuurdatabank.be/XMLSchema/region_nat" xmlns:rel="http://www.cultuurdatabank.be/XMLSchema/relation_item" xmlns:sp="http://www.cultuurdatabank.be/XMLSchema/street_part" xmlns:uc="http://www.cultuurdatabank.be/XMLSchema/url_comp" xmlns:zc="http://www.cultuurdatabank.be/XMLSchema/zip_codes" xmlns:java="http://xml.apache.org/xalan/java" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ccc="http://www.cultuurdatabank.be/XMLSchema/character_codes" xmlns:extfun="external_functions" xmlns:file="java.io.File" xmlns:xalan="http://xml.apache.org/xalan" exclude-result-prefixes="cc sc ad ca ci ct co ec ed gc lc mc me md na no ot or pe ph pr ref rn rel sp uc zc xs xalan java extfun file ccc">
	...
	<xalan:component prefix="extfun">
		...
		// ISO-8859-1  
		// Need to use a InputStreamReader on a FileInputStream so ISO-8859-1 can be specified.
		...
		function get_text_file_char_old(infilename)
		 {
		
		// This still uses unicode to interpret the characters so don't use it 
		// BufferedReader in = new BufferedReader(new FileReader("infilename"));
		var file = new java.io.FileReader(infilename)
		var input = new java.io.BufferedReader(file)
		...
		while ((str = input.read()) != -1) {
			 all_text.append(str).toString()
			// note that read line dos not copy line termination characters so add a cr/lf at the end of every line
			all_text.append(',').toString()
			//java.lang.System.out.println(str);
			//java.lang.System.out.println("xxxxxxxxxx");
			   
        }
         //java.lang.System.out.println(all_text);
		...
		return all_text
		 ...
		 function get_text_file_old(infilename)
		 {
		// This still uses unicode to interpret the characters so don't use it 
		// BufferedReader in = new BufferedReader(new FileReader("infilename"));
				...
				<events>
					...
					<xsl:for-each select="//event_cluster_instance[event_cluster_type='aanbod']">
						...
						<xsl:if test="string(organisation) != '' and string(ot:opening_times) != '' and string($organisator_exists) = 'true' and
										string($location_address) = 'true' and string($bad_opening_times) = '' and string($bad_opening_times_1) = 'true'">
							<event>
												...
												<xsl:when test="substring(./file,($media_length)-3,4) = '.txt'">
													...
													<xsl:if test="string(./value_plain_text/dutch) != 'kinderen toegelaten'">
														...
														<xsl:if test="string(./category/categorisation_instance/label/dutch) != 'prijsomschrijvingen en kortingen'">
															...
															<!-- for iis -->
															<!--<xsl:variable name="full_text_file" select="concat($iis.ext.files,'/',$text_file)"/> -->
															<!-- for test : -->
															<!-- <xsl:variable name="full_text_file" select="concat('T:\XML\brugge\character_problem\CDB00001_200802230400_0639_D\',$text_file)"/>  -->
															<xsl:variable name="full_text_file" select="concat('T:\XML\brugge\character_problem\CDB00001_200802230400_0639_D\',$text_file)"/>
															...
															<xsl:if test="position() = 1">
																<xsl:choose>
																	<xsl:when test="(file:exists($file))">
																		<longdescription>
																			<xsl:variable name="character_string">
																				<xsl:value-of select="extfun:get_text_file($full_text_file)"/>
																			</xsl:variable>
																			<xsl:value-of select="$character_string"/>
																		</longdescription>
																	</xsl:when>
																	<xsl:otherwise>
...
-->
	...
	<!-- not used -->
	<xsl:template name="decimal_characters">
		<xsl:param name="string"/>
		<xsl:choose>
			<xsl:when test="string-length($string) &gt; 0">
				<xsl:variable name="single_char" select="substring-before($string,',')"/>
				<xsl:variable name="converted_char" select="document('')/*/ccc:Import/ccc:Row[@Dec= $single_char]/@Char"/>
				<xsl:value-of select="$converted_char"/>
				<xsl:variable name="new_string" select="substring-after($string,',')"/>
				<xsl:call-template name="decimal_characters">
	...
	<ccc:Import>
		<ccc:Row Dec="32.0" Char=" "/>
		<ccc:Row Dec="33.0" Char="!"/>
		<ccc:Row Dec="34.0" Char="&quot;"/>
		<ccc:Row Dec="35.0" Char="#"/>
		<ccc:Row Dec="36.0" Char="$"/>
		<ccc:Row Dec="37.0" Char="%"/>
		<ccc:Row Dec="38.0" Char="&amp;"/>
		<ccc:Row Dec="39.0" Char="&apos;"/>
		<ccc:Row Dec="40.0" Char="("/>
		<ccc:Row Dec="41.0" Char=")"/>
		<ccc:Row Dec="42.0" Char="*"/>
		<ccc:Row Dec="43.0" Char="+"/>
		<ccc:Row Dec="44.0" Char=","/>
		<ccc:Row Dec="45.0" Char="-"/>
		<ccc:Row Dec="46.0" Char="."/>
		<ccc:Row Dec="47.0" Char="/"/>
		<ccc:Row Dec="48.0" Char="0"/>
		<ccc:Row Dec="49.0" Char="1"/>
		<ccc:Row Dec="50.0" Char="2"/>
		<ccc:Row Dec="51.0" Char="3"/>
		<ccc:Row Dec="52.0" Char="4"/>
		<ccc:Row Dec="53.0" Char="5"/>
		<ccc:Row Dec="54.0" Char="6"/>
		<ccc:Row Dec="55.0" Char="7"/>
		<ccc:Row Dec="56.0" Char="8"/>
		<ccc:Row Dec="57.0" Char="9"/>
		<ccc:Row Dec="58.0" Char=":"/>
		<ccc:Row Dec="59.0" Char=";"/>
		<ccc:Row Dec="60.0" Char="&lt;"/>
		<ccc:Row Dec="61.0" Char="="/>
		<ccc:Row Dec="62.0" Char="&gt;"/>
		<ccc:Row Dec="63.0" Char="?"/>
		<ccc:Row Dec="64.0" Char="@"/>
		<ccc:Row Dec="65.0" Char="A"/>
		<ccc:Row Dec="66.0" Char="B"/>
		<ccc:Row Dec="67.0" Char="C"/>
		<ccc:Row Dec="68.0" Char="D"/>
		<ccc:Row Dec="69.0" Char="E"/>
		<ccc:Row Dec="70.0" Char="F"/>
		<ccc:Row Dec="71.0" Char="G"/>
		<ccc:Row Dec="72.0" Char="H"/>
		<ccc:Row Dec="73.0" Char="I"/>
		<ccc:Row Dec="74.0" Char="J"/>
		<ccc:Row Dec="75.0" Char="K"/>
		<ccc:Row Dec="76.0" Char="L"/>
		<ccc:Row Dec="77.0" Char="M"/>
		<ccc:Row Dec="78.0" Char="N"/>
		<ccc:Row Dec="79.0" Char="O"/>
		<ccc:Row Dec="80.0" Char="P"/>
		<ccc:Row Dec="81.0" Char="Q"/>
		<ccc:Row Dec="82.0" Char="R"/>
		<ccc:Row Dec="83.0" Char="S"/>
		<ccc:Row Dec="84.0" Char="T"/>
		<ccc:Row Dec="85.0" Char="U"/>
		<ccc:Row Dec="86.0" Char="V"/>
		<ccc:Row Dec="87.0" Char="W"/>
		<ccc:Row Dec="88.0" Char="X"/>
		<ccc:Row Dec="89.0" Char="Y"/>
		<ccc:Row Dec="90.0" Char="Z"/>
		<ccc:Row Dec="91.0" Char="["/>
		<ccc:Row Dec="92.0" Char="\"/>
		<ccc:Row Dec="93.0" Char="]"/>
		<ccc:Row Dec="94.0" Char="^"/>
		<ccc:Row Dec="95.0" Char="_"/>
		<ccc:Row Dec="96.0" Char="`"/>
		<ccc:Row Dec="97.0" Char="a"/>
		<ccc:Row Dec="98.0" Char="b"/>
		<ccc:Row Dec="99.0" Char="c"/>
		<ccc:Row Dec="100.0" Char="d"/>
		<ccc:Row Dec="101.0" Char="e"/>
		<ccc:Row Dec="102.0" Char="f"/>
		<ccc:Row Dec="103.0" Char="g"/>
		<ccc:Row Dec="104.0" Char="h"/>
		<ccc:Row Dec="105.0" Char="i"/>
		<ccc:Row Dec="106.0" Char="j"/>
		<ccc:Row Dec="107.0" Char="k"/>
		<ccc:Row Dec="108.0" Char="l"/>
		<ccc:Row Dec="109.0" Char="m"/>
		<ccc:Row Dec="110.0" Char="n"/>
		<ccc:Row Dec="111.0" Char="o"/>
		<ccc:Row Dec="112.0" Char="p"/>
		<ccc:Row Dec="113.0" Char="q"/>
		<ccc:Row Dec="114.0" Char="r"/>
		<ccc:Row Dec="115.0" Char="s"/>
		<ccc:Row Dec="116.0" Char="t"/>
		<ccc:Row Dec="117.0" Char="u"/>
		<ccc:Row Dec="118.0" Char="v"/>
		<ccc:Row Dec="119.0" Char="w"/>
		<ccc:Row Dec="120.0" Char="x"/>
		<ccc:Row Dec="121.0" Char="y"/>
		<ccc:Row Dec="122.0" Char="z"/>
		<ccc:Row Dec="123.0" Char="{"/>
		<ccc:Row Dec="124.0" Char="|"/>
		<ccc:Row Dec="125.0" Char="}"/>
		<ccc:Row Dec="126.0" Char="~"/>
		<ccc:Row Dec="127.0" Char="_"/>
		<ccc:Row Dec="128.0" Char="?"/>
		<ccc:Row Dec="129.0" Char="?"/>
		<ccc:Row Dec="130.0" Char="?"/>
		<ccc:Row Dec="131.0" Char="?"/>
		<ccc:Row Dec="132.0" Char="?"/>
		<ccc:Row Dec="133.0" Char="?"/>
		<ccc:Row Dec="134.0" Char="?"/>
		<ccc:Row Dec="135.0" Char="?"/>
		<ccc:Row Dec="136.0" Char="?"/>
		<ccc:Row Dec="137.0" Char="?"/>
		<ccc:Row Dec="138.0" Char="?"/>
		<ccc:Row Dec="139.0" Char="?"/>
		<ccc:Row Dec="140.0" Char="?"/>
		<ccc:Row Dec="141.0" Char="?"/>
		<ccc:Row Dec="142.0" Char="?"/>
		<ccc:Row Dec="143.0" Char="?"/>
		<ccc:Row Dec="144.0" Char="?"/>
		<ccc:Row Dec="145.0" Char="?"/>
		<ccc:Row Dec="146.0" Char="?"/>
		<ccc:Row Dec="147.0" Char="?"/>
		<ccc:Row Dec="148.0" Char="?"/>
		<ccc:Row Dec="149.0" Char="?"/>
		<ccc:Row Dec="150.0" Char="?"/>
		<ccc:Row Dec="151.0" Char="?"/>
		<ccc:Row Dec="152.0" Char="?"/>
		<ccc:Row Dec="153.0" Char="?"/>
		<ccc:Row Dec="154.0" Char=""/>
		<ccc:Row Dec="155.0" Char="?"/>
		<ccc:Row Dec="156.0" Char="°"/>
		<ccc:Row Dec="157.0" Char="²"/>
		<ccc:Row Dec="158.0" Char="·"/>
		<ccc:Row Dec="159.0" Char="÷"/>
		<ccc:Row Dec="160.0" Char="?"/>
		<ccc:Row Dec="161.0" Char="?"/>
		<ccc:Row Dec="162.0" Char="?"/>
		<ccc:Row Dec="163.0" Char="?"/>
		<ccc:Row Dec="164.0" Char="?"/>
		<ccc:Row Dec="165.0" Char="?"/>
		<ccc:Row Dec="166.0" Char="?"/>
		<ccc:Row Dec="167.0" Char="?"/>
		<ccc:Row Dec="168.0" Char="?"/>
		<ccc:Row Dec="169.0" Char="?"/>
		<ccc:Row Dec="170.0" Char="?"/>
		<ccc:Row Dec="171.0" Char="?"/>
		<ccc:Row Dec="172.0" Char="?"/>
		<ccc:Row Dec="173.0" Char="?"/>
		<ccc:Row Dec="174.0" Char="?"/>
		<ccc:Row Dec="175.0" Char="?"/>
		<ccc:Row Dec="176.0" Char="?"/>
		<ccc:Row Dec="177.0" Char="?"/>
		<ccc:Row Dec="178.0" Char="?"/>
		<ccc:Row Dec="179.0" Char="?"/>
		<ccc:Row Dec="180.0" Char="?"/>
		<ccc:Row Dec="181.0" Char="?"/>
		<ccc:Row Dec="182.0" Char="?"/>
		<ccc:Row Dec="183.0" Char="?"/>
		<ccc:Row Dec="184.0" Char="?"/>
		<ccc:Row Dec="185.0" Char="?"/>
		<ccc:Row Dec="186.0" Char="?"/>
		<ccc:Row Dec="187.0" Char="?"/>
		<ccc:Row Dec="188.0" Char="?"/>
		<ccc:Row Dec="189.0" Char="?"/>
		<ccc:Row Dec="190.0" Char="?"/>
		<ccc:Row Dec="191.0" Char="©"/>
		<ccc:Row Dec="192.0" Char="?"/>
		<ccc:Row Dec="193.0" Char="?"/>
		<ccc:Row Dec="194.0" Char="?"/>
		<ccc:Row Dec="195.0" Char="?"/>
		<ccc:Row Dec="196.0" Char="?"/>
		<ccc:Row Dec="197.0" Char="?"/>
		<ccc:Row Dec="198.0" Char="?"/>
		<ccc:Row Dec="199.0" Char="?"/>
		<ccc:Row Dec="200.0" Char="?"/>
		<ccc:Row Dec="201.0" Char="?"/>
		<ccc:Row Dec="202.0" Char="?"/>
		<ccc:Row Dec="203.0" Char="?"/>
		<ccc:Row Dec="204.0" Char="?"/>
		<ccc:Row Dec="205.0" Char="?"/>
		<ccc:Row Dec="206.0" Char="?"/>
		<ccc:Row Dec="207.0" Char="?"/>
		<ccc:Row Dec="208.0" Char="?"/>
		<ccc:Row Dec="209.0" Char="?"/>
		<ccc:Row Dec="210.0" Char="?"/>
		<ccc:Row Dec="211.0" Char="?"/>
		<ccc:Row Dec="212.0" Char="?"/>
		<ccc:Row Dec="213.0" Char="?"/>
		<ccc:Row Dec="214.0" Char="?"/>
		<ccc:Row Dec="215.0" Char="?"/>
		<ccc:Row Dec="216.0" Char="?"/>
		<ccc:Row Dec="217.0" Char="?"/>
		<ccc:Row Dec="218.0" Char="?"/>
		<ccc:Row Dec="219.0" Char="?"/>
		<ccc:Row Dec="220.0" Char="?"/>
		<ccc:Row Dec="221.0" Char="?"/>
		<ccc:Row Dec="222.0" Char="?"/>
		<ccc:Row Dec="223.0" Char=""/>
		<ccc:Row Dec="224.0" Char="?"/>
		<ccc:Row Dec="225.0" Char="?"/>
		<ccc:Row Dec="226.0" Char="?"/>
		<ccc:Row Dec="227.0" Char="?"/>
		<ccc:Row Dec="228.0" Char="?"/>
		<ccc:Row Dec="229.0" Char="?"/>
		<ccc:Row Dec="230.0" Char="?"/>
		<ccc:Row Dec="231.0" Char="?"/>
		<ccc:Row Dec="232.0" Char="?"/>
		<ccc:Row Dec="233.0" Char="&#233;"/>
		<ccc:Row Dec="234.0" Char="?"/>
		<ccc:Row Dec="235.0" Char="?"/>
		<ccc:Row Dec="236.0" Char="?"/>
		<ccc:Row Dec="237.0" Char="?"/>
		<ccc:Row Dec="238.0" Char="?"/>
		<ccc:Row Dec="239.0" Char="?"/>
		<ccc:Row Dec="240.0" Char="?"/>
		<ccc:Row Dec="241.0" Char="?"/>
		<ccc:Row Dec="242.0" Char="?"/>
		<ccc:Row Dec="243.0" Char="?"/>
		<ccc:Row Dec="244.0" Char="?"/>
		<ccc:Row Dec="245.0" Char="?"/>
		<ccc:Row Dec="246.0" Char="?"/>
		<ccc:Row Dec="247.0" Char="?"/>
		<ccc:Row Dec="248.0" Char="?"/>
		<ccc:Row Dec="249.0" Char="?"/>
		<ccc:Row Dec="250.0" Char="?"/>
		<ccc:Row Dec="251.0" Char="?"/>
		<ccc:Row Dec="252.0" Char="?"/>
		<ccc:Row Dec="253.0" Char="?"/>
		<ccc:Row Dec="254.0" Char="?"/>
		<ccc:Row Dec="255.0" Char="?"/>
	</ccc:Import>
</xsl:stylesheet>
