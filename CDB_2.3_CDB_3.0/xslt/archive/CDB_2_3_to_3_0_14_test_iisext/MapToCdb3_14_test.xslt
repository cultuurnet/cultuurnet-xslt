...
\\172.16.1.51\xml\XSD\CDB_3_0\CDBXML~1.0-F\CdbXSD.xsd</xsl:attribute>-->
		...
		http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.0/FINAL/CdbXSD.xsd</xsl:attribute>
			...
			<metadata>
				<versions>
					<categorisation>
					<xsl:attribute name="url"><xsl:value-of select="'http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.0/FINAL/categories/1.11/categorisation.xml'"/></xsl:attribute>
					</categorisation>
					<regions>
											...
											<xsl:choose>
												<xsl:when test="substring(./file,($media_length)-3,4) = '.txt'">
													...
													<xsl:if test="string(./value_plain_text/dutch) != 'kinderen toegelaten'">
														...
														<xsl:variable name="full_text_file" select="concat('T:\XML\CDB_3_0_Antwerp_Region\Antwerpen_Region_200804071050_0016_F\',$text_file)"/>
														-->
															<!-- for iis 
														...
														-->
															...
															<longdescription>
															full text file : <xsl:value-of select="$full_text_file"></xsl:value-of>
															</longdescription>
															...
															file exists : <xsl:value-of select="(file:exists($file))"></xsl:value-of>
																<xsl:choose>
																	
																	<xsl:when test="(file:exists($file))">
																		<!-- <longdescription> -->
																			longdescription : <xsl:value-of select="extfun:get_text_file($full_text_file)"/>
																		<!-- </longdescription> -->
																	</xsl:when>
																	<xsl:otherwise>
																	use plain text
