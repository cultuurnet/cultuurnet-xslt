...
<!--
Updated on 27/08/10 to add the externalid prefix to the image filenamecopied to the uitdb ftp site  to ensure it is unique.
This xslt also adds the externalid prefix to the  fle name in the xml.

This version for 3.1 Final = IIS3.2
...
<xsl:stylesheet version="1.0" xmlns="http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.1/FINAL" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:n1="http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.1/FINAL" xmlns:java="http://xml.apache.org/xalan/java" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:extfun="external_functions" xmlns:file="java.io.File" xmlns:url="java.net.URL" xmlns:stream="java.io.OutputStreamWriter" xmlns:xalan="http://xml.apache.org/xalan" exclude-result-prefixes="n1  xalan java extfun file">
	...
	<xsl:template match="/n1:cdbxml">
		<cdbxml>
			<xsl:attribute name="xsi:schemaLocation">http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.1/FINAL
		http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.1/FINAL/CdbXSD.xsd</xsl:attribute>
			...
			<!-- if the filename is a reference then leave it and do nothing. -->
			...
			<xsl:for-each select="//n1:filename">
				...
				<!-- <xsl:variable name="full_filename_from" select="concat($iis.publication.externalIdPrefix,'_',$filename_from)"/> -->
				 <xsl:variable name="full_filename_from" select="concat($local_dir_3,$filename_from)"/>
				<!--   <xsl:variable name="filename" select="concat('ftp://media:cnET2008Udb@media.cultuurnet.be/partnermedia','/',$iis.publication.externalIdPrefix,'_',$filename_from)"/>  -->
				  <xsl:variable name="filename" select="concat('ftp://media:cnET2008Udb@media.cultuurnet.be/partnermedia','/',$iis.publication.id,'_',$filename_from)"/> 
				
				<xsl:variable name="file" select="file:new($filename)"/>
				<xsl:variable name="file_in" select="file:new($full_filename_from)"/>
			 	<xsl:if test="file:exists($file_in)">
	...
	<xsl:template match="/n1:filename">
