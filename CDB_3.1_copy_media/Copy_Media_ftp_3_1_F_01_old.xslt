...
This version for 3.1 Final = IIS 3.2
	...
	<xsl:template match="n1:file">
		<file>
			...
			<xsl:if test="not(file:exists($file))">
				...
				<xsl:if test="extfun:writeLoopjpg_test($full_filename_from,translate($filename,' ','_'))"/>
			...
			<xsl:if test="string(n1:filename) != '' or substring(n1:hlink,1,4) != 'http'">
			<filename>
				<xsl:choose>
