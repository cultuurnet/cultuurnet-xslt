...
<!--
Used for krokus Krokuskriebels to replace the spaces in the filenames
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx		 
	...
	<xsl:template match="n1:file">
		<file>
			...
			<!-- <xsl:if test="not(file:exists($file)) or file:length($file)=0 "> temp  --> 
				...
				<!-- paramaters : file_in, file_out -->
				<!-- for KK filename_in is OK already -->
				
				<!--   old from scarab <xsl:if test="extfun:filetofile_http(translate($http_filename_new,' ','%20'),translate($filename,' ','_'))"></xsl:if>  -->
				
				<!-- new for kk -->
				<xsl:variable name="filename_kk">
					<xsl:call-template name="replace_space_20">
						<xsl:with-param name="text_string" select="$filename"/>
					</xsl:call-template>
				</xsl:variable>
				
				<xsl:if test="extfun:filetofile_http(translate($http_filename_new,' ','%20'),translate($filename_kk,' ','_'))">
			...
			<filename>
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:call-template name="replace_space_20">
						<xsl:with-param name="text_string" select="$scarab_filename"/>
					</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>
			</filename>
			...
			<xsl:variable name="hlink_filename">
					<xsl:call-template name="replace_space_20">
						<xsl:with-param name="text_string" select="concat('ftp://ftp.045:Kihuesac@ftp.iis.uitdatabank.be/mediafolder','/',$filename_from_plus_ext_id)"/>
					</xsl:call-template>
				</xsl:variable>
	...
	<xsl:template name="replace_space">
		...
		<xsl:template name="replace_space_20">
		<xsl:param name="text_string"/>
		<xsl:choose>
			<xsl:when test="contains($text_string,'%20')">
				<xsl:value-of select="substring-before($text_string,'%20')"/>
				<xsl:value-of select="'_'"/>
				<xsl:call-template name="replace_space_20">
					<xsl:with-param name="text_string" select="substring-after($text_string,'%20')"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$text_string"/>
