...
<!--
Use tis one if the media can contain reservations
-->
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx		 
 function filetofile_http_test(file_in,file_out)
		 {
		...
		//------------------------------------------------------------ 
		 ...
		 function filetofile_http_(file_in,file_out)
		 {
		 var url = new java.net.URL(file_in);
	...
	<xsl:template match="n1:file">
		<xsl:choose>
		<xsl:when test="n1:mediatype = 'reservations'"><xsl:copy-of select="."></xsl:copy-of></xsl:when>
		<xsl:otherwise>
		<file>
			<xsl:variable name="local_dir_1">
				<!-- <xsl:value-of select="java:org.apache.xalan.lib.NodeInfo.systemId()"/> -->
				<!-- for test -->
				<xsl:value-of select="'C:/XML/XML/Revolver'"></xsl:value-of>
			</xsl:variable>
			<xsl:variable name="local_dir_2">
				<xsl:choose>
					...
					<xsl:when test="contains($local_dir_1,'C:/XML/XML')">
						...
						<xsl:value-of select="$local_dir_1"></xsl:value-of>
					</xsl:when>
			...
			<xsl:value-of select="$local_dir_3"></xsl:value-of>
			<!--
			...
			<!-- for scarab the filename is the url or can be empty so we need to strip off att the characters up to and including the last slash -->
			<xsl:variable name="scarab_filename">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:call-template name="string_after_last_slash">
							<xsl:with-param name="text_string" select="n1:hlink"/>
						</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			...
			<!-- test   xxx <xsl:value-of select="file:length($file)"></xsl:value-of>zzz  -->
			 <xsl:if test="not(file:exists($file)) or file:length($file)=0 "> 
				<xsl:variable name="http_filename_new">
					<xsl:call-template name="replace_space">
						<xsl:with-param name="text_string" select="$http_filename"/>
					</xsl:call-template>
				</xsl:variable>
				...
				www<xsl:value-of select="$file"></xsl:value-of>www
				xxx<xsl:value-of select="$full_filename_from"></xsl:value-of>xxx
					yyy<xsl:value-of select="$filename"></xsl:value-of>yyy
				zzz<xsl:value-of select="$http_filename_new"></xsl:value-of>zzz
				
				
				
				
				
				<xsl:variable name="http_filename_new_http">
					 <xsl:value-of select="substring($http_filename_new,1,4)"></xsl:value-of>	
					  <xsl:value-of select="substring($http_filename_new,6,string-length($http_filename_new)-5)"></xsl:value-of>	
				</xsl:variable>
				bbb<xsl:value-of select="$http_filename_new_http"></xsl:value-of>bbb
				<xsl:if test="extfun:filetofile_http_test(translate($http_filename_new_http,' ','%20'),$full_filename_from)"></xsl:if>
			<!--<xsl:if test="extfun:filetofile_http(translate($http_filename_new,' ','%20'),translate($filename,' ','_'))">  </xsl:if>  original-->
			</xsl:if>
			<xsl:copy-of select="n1:copyright"/>
			<xsl:variable name="media_filename">
				<xsl:choose>
					...
					<xsl:otherwise>
						<!--2014/02/10 : UDB-1627 Images komen niet door : We need the file name with the externalid prefix -->
						<xsl:value-of select="translate($filename_from_plus_ext_id,' ','_')"/>
						<!--<xsl:value-of select="$scarab_filename"/> -->
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:if test="string($media_filename) != ''">
				<filename>
					<xsl:value-of select="$media_filename"/>
				</filename>
			</xsl:if>
			...
			<mediatype>
				<xsl:choose>
					<xsl:when test="n1:mediatype = 'imageweb'">
						<xsl:value-of select="'photo'"/>
					</xsl:when>
					<xsl:when test="string(n1:mediatype) = ''">
						<xsl:value-of select="'photo'"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="n1:mediatype"/>
					</xsl:otherwise>
				</xsl:choose>
			</mediatype>
