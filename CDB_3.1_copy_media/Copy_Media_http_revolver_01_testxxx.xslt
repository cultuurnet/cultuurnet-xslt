	...
	<xsl:template match="n1:file">
		...
		<file>
			...
			<xsl:if test="not(file:exists($file)) or file:length($file)=0 ">
				...
				<xsl:if test="extfun:filetofile_http(translate($http_filename_new,' ','%20'),translate($filename,' ','_'))"> </xsl:if>
			<!--<xsl:if test="extfun:filetofile_http(translate($http_filename_new,' ','%20'),translate($filename,' ','_'))"> 
				 </xsl:if> -->
			</xsl:if>
