...
Always use this as the firts xslt as it only works on CDB XML 3.1

It copies the xml with no modifications so it can be used at the output.
It then copies media files from their href to directory where the xml file is or from the IIS3 local media directory.
-->
...
return true;
	...
	<xsl:template match="/n1:cdbxml">
		...
		http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.1/DRAFT/CdbXSD.xsd</xsl:attribute>
			...
			<!-- find out where we are going to copy to -->
			...
			<!-- note that if we don't find the file via the hlink then we will look in iis.ext.dir with just the file name	-->
			<!-- that is where new files from partners such as SKB are put																-->
			<xsl:for-each select="//n1:hlink">
				<xsl:variable name="url" select="."/>
				<xsl:variable name="local_filename" select="../n1:filename"/>
				<!-- <xsl:variable name="local_filename">
					<xsl:call-template name="string_after_last_slash">
						<xsl:with-param name="text_string" select="$url"/>
					</xsl:call-template>
				</xsl:variable> -->
				<xsl:variable name="filename" select="concat($local_dir_3,$local_filename)"/>
				<xsl:variable name="file" select="file:new($filename)"/>
				<!--
				<xsl:if test="not(file:exists($file))">
					<xsl:if test="extfun:writeLoopjpg(string($url),$filename)"/>
				</xsl:if>
				-->
				<xsl:variable name="file_length" select="string-length($file)"/>
				<!-- this was going to be a temporary fix. Didn't need it as I added try catch statements to the writelooping and filetofile functions 
				<xsl:choose>
					<xsl:when test="substring($file,$file_length -3,4) = '.com' or substring($file,$file_length -2,3) = '.be' or substring($file,$file_length -3,4) = '.be/' or substring($file,$file_length -2,3) = 's/7'"/>
					<xsl:otherwise> -->
					<!-- test
				xxxxx<xsl:value-of select="$file"/>
				yyyyy<xsl:value-of select="$url"/>
				zzzzz<xsl:value-of select="$filename"/> -->
				
				<!-- replace the spaces in filenames with %20 -->
				<xsl:variable name="new_url">
					<xsl:call-template name="replace_space">
					<xsl:with-param name="text_string" select="$url"/>
				</xsl:call-template>
				</xsl:variable>
				...
				<xsl:if test="substring($new_url,string-length($new_url)-2,3)= 'jpg' or
				substring($new_url,string-length($new_url)-2,3)= 'zip' or
				substring($new_url,string-length($new_url)-3,4)= 'jpeg' or
				substring($new_url,string-length($new_url)-2,3)= 'png' or
				substring($new_url,string-length($new_url)-2,3)= 'gif' or
				substring($new_url,string-length($new_url)-2,3)= 'rtf' or
				substring($new_url,string-length($new_url)-2,3)= 'doc' or
				substring($new_url,string-length($new_url)-2,3)= 'xls'">
				...
				<xsl:if test="not(file:exists($file))">
					<xsl:choose>
						<xsl:when test="extfun:writeLoopjpg(string($new_url),$filename)">
						
						</xsl:when>
						<xsl:otherwise>
					
							<!-- file not in the CDB so try the local directory -->
							<!-- for IIS3 -->
							<!--  <xsl:variable name="full_filename_from_local" select="concat($iis.ext.dir,'/',$local_filename)"/>     -->
							<!-- for testing -->
							<!-- <xsl:variable name="full_filename_from_local" select="concat('D:\XML\XML\IIS_3_Media_test\sub_test_1','/','test.jpg')"/> -->
							<xsl:variable name="full_filename_from_local" select="concat($iis.ext.dir,'/',$local_filename)"/>
							<!--
							<xsl:value-of select="$full_filename_from_local"></xsl:value-of>
							<xsl:value-of select="$filename"></xsl:value-of>
							-->
							<xsl:if test="not(file:exists($file))">
							
								<xsl:if test="extfun:filetofile($full_filename_from_local,$filename)"></xsl:if>
							</xsl:if>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
			...
			<!-- long descriptions will be copied for 3.1 to 2.3 transformations. They are here for the moment as a test 
			<xsl:for-each select="//n1:longdescription">
				<xsl:variable name="content" select="."/>
				<xsl:variable name="local_filename">
					<xsl:value-of select="concat(../../../@cdbid,'.txt')"/>
				</xsl:variable>
				<xsl:variable name="filename" select="concat($local_dir_3,$local_filename)"/>
				<xsl:variable name="file" select="java:java.io.BufferedWriter.new(java:java.io.FileWriter.new($filename))"/>
				<xsl:variable name="open" select="java:write($file, $content)"/>
				<xsl:variable name="close" select="java:close($file)"/>
			</xsl:for-each>  -->
		</cdbxml>
	</xsl:template>
	...
	<xsl:template name="replace_space">
		<xsl:param name="text_string"/>
		<xsl:choose>
			<xsl:when test="contains($text_string,' ')">
				<xsl:value-of select="substring-before($text_string,' ')"/>
