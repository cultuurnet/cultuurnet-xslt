...
return true;
	...
	<xsl:template match="/n1:cdbxml">
		...
		http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.1/DRAFT/CdbXSD.xsd</xsl:attribute>
			...
			<!-- if the media file is in the input zip file then copy it to a local file -->
			<!-- find out where we are going to copy to -->
			<!-- eventually this will be what is the IIS3 variable -->
			<!--for first test copy them back to the local dir with a different file name. -->
			<!-- use the test tag to force the produced xml to be invalid. Then we can copy it from IIS3 -->
			<!-- <test_tag>aaaaaaaaa</test_tag> -->
			<xsl:for-each select="//n1:filename">
				...
				<!-- strange : when I use this it doesn't work ! 
				<xsl:variable name="filename">
					<xsl:choose>
						<xsl:when test="string-length($iis.ext.dir) = 0">
							<xsl:value-of select="concat($local_dir_3,'xxx',$local_filename)"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat($iis.ext.dir,'/',$local_filename)"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
					-->
				...
				<!-- for testing -->
				<!-- <xsl:variable name="filename" select="concat($local_dir_3,'xxx',$filename_from))"/> -->
				<!-- for IIS3 -->
				...
				<xsl:if test="not(file:exists($file))">
					<!--<xsl:if test="extfun:writeLoopjpg(string($url),$filename)"/> 			this copies from url to a file name.-->
					<!-- we need to copy from file to url -->
					<!--xxx	<xsl:value-of select="$filename"></xsl:value-of>...
									<xsl:value-of select="$full_filename_from"></xsl:value-of>	zzz-->
					<xsl:if test="extfun:filetofile($full_filename_from,$filename)"/>
				</xsl:if>
			</xsl:for-each>
