...
-updated 20/05/2015
-added performer, catid, reservation, contact mail address
-see MDH-83 extends transform list 4 with new fields
-xml in C:\XML\XML\CDB_XML_3_2_test

-updated 08/08/16
added theme
see https://jira.uitdatabank.be/browse/MDH-88
			...
			<events>
				<xsl:for-each select="cdb:event">
					<item>
						...
						<!-- added 17-11-14-->
						<xsl:attribute name="permanent"><xsl:choose><xsl:when test="string(cdb:calendar/cdb:permanentopeningtimes) != ''"><xsl:value-of select="'true'"/></xsl:when><xsl:otherwise><xsl:value-of select="'false'"/></xsl:otherwise></xsl:choose></xsl:attribute>
						...
						<xsl:attribute name="performer"><xsl:for-each select="cdb:eventdetails/cdb:eventdetail/cdb:performers/cdb:performer"><xsl:value-of select="cdb:label"/><xsl:if test="position() != last()"><xsl:value-of select="';'"/></xsl:if></xsl:for-each></xsl:attribute>
						<xsl:attribute name="region">
							<xsl:variable name="region">
								<xsl:for-each select="cdb:categories">
									<xsl:for-each select="cdb:category">
										<xsl:if test="@type = 'flandersregion'">
							...
							<xsl:value-of select="substring($region,1,string-length($region)-1)"></xsl:value-of>
						</xsl:attribute>
						<xsl:attribute name="reservation">
						<xsl:variable name="reservation">
							<xsl:for-each select="cdb:contactinfo/cdb:url">
								<xsl:if test="@reservation = 'true'">
									<xsl:value-of select="."/>
									<xsl:value-of select="';'"/>
								</xsl:if>
							</xsl:for-each>
							<xsl:for-each select="cdb:contactinfo/cdb:mail">
								<xsl:if test="@reservation = 'true'">
									<xsl:value-of select="."/>
									<xsl:value-of select="';'"/>
								</xsl:if>
							...
							<xsl:value-of select="substring($reservation,1,string-length($reservation)-1)"></xsl:value-of>
						</xsl:attribute>
						<xsl:attribute name="contact">
						<xsl:variable name="contact">
						<!-- street and housenr optional 
							...
							<!--xxx<xsl:value-of select="@reservation"></xsl:value-of>zzz
							...
							<xsl:value-of select="substring($contact,1,string-length($contact)-1)"></xsl:value-of>
						</xsl:attribute>
					</item>
				</xsl:for-each>
			</events>
