...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:n1="http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.3/FINAL" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.2/FINAL" xmlns:ns3="http://www.w3.org/2001/XMLSchema" xmlns:cdb="http://www.cultuurdatabank.com/XMLSchema/CdbXSD/3.2/FINAL" exclude-result-prefixes="n1 xs ns3">
	...
	<xsl:template match="n1:cdbxml">
		<cdbxml>
			...
			<xsl:for-each select="n1:facets">
				<facets>
					<xsl:apply-templates select="@*"/>
					<xsl:apply-templates select="node()"/>
				</facets>
			</xsl:for-each>
			<xsl:for-each select="n1:suggestions">
				<suggestions>
					<xsl:apply-templates select="@*"/>
					<xsl:apply-templates select="node()"/>
				</suggestions>
			</xsl:for-each>
			<xsl:for-each select="n1:cities">
				<cities>
					<xsl:apply-templates select="@*"/>
					<xsl:apply-templates select="node()"/>
				</cities>
			</xsl:for-each>
			<!-- JCO : end added 23/March 2015 -->
		</cdbxml>
	</xsl:template>
